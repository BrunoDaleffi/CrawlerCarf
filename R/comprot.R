...
get_comprot <- function(id) {
  ...
  purrr::map(id, get_comprot_) %>%
    purrr::keep(function(x){tibble::is.tibble(x)}) %>%
    purrr::reduce(bind_rows) 
}

#' Get lawsuits' comunication and protocol data (comprot) from future
#'
#' @param id Character vector with one or more lawsuit IDs
#' @return A tibble with the following columns:
#' \itemize{
...
#'
#' @export
future_get_comprot <- function(id) {
  
  # Clean IDs
  id <- stringr::str_replace_all(id, "[^0-9]", "")
  
  # Iterate over IDs
  furrr::future_map(id, get_comprot_) %>%
    purrr::keep(function(x){tibble::is.tibble(x)}) %>%
    purrr::reduce(bind_rows) 
}
